...
visualization of continous signal data (typically enrichment in IP) via 
clustered heatmaps and track-like line plots.  
...
There is no hard limit to the number of sets *seqsetvis* can show in an Euler diagram
...
Ellipse variant of Euler diagram.  Ellipses are capable of better fitting the
set overlaps.
```{r euler ellipse}
...
```{r ctcf scatterplots, fig.width=12, fig.height=7}
...
```{r ctcf recentered heatmap, message = FALSE, fig.width=14, fig.height=6}
center_gr = centerAtMax(clust_gr, view_size = 150, 
                        by_ = "id", check_by_dupes = FALSE)


p_center_hmap = ssvSignalHeatmap(center_gr) + labs(fill = "FE",
                                  y = "region",
                                  x = "bp from center")
...
p_center_hmap_reclust = ssvSignalHeatmap(clust_center_gr) + labs(fill = "FE",
                                  y = "region",
                                  x = "bp from center")
...
if(dir.exists(cache_path) & require(BiocFileCache)){
    ...
    state_gr = resize(state_gr, width = chmm_win + 2 * flank_size, fix = "center")
...
enrichment = state_peak_fraction / state_width_fraction[names(state_peak_fraction)]
...
ggdraw() +
    ...
    draw_plot_label(LETTERS[1:4], 
                    c(0, 0.48, 0, 0), 
                    c(1, 1, 1 - bar_height, 1 - bar_height - line_height), size = 15)
```
