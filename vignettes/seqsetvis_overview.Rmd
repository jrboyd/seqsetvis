...
bigWig format.  This is done via the `ssvFetchBigwig()` and 
`ssvFetchBigwigList()` functions.
...
bw_gr = ssvFetchBigwigList(bigwig_files, olap_gr, win_size = window_size)
...
```{r fetch package bw}
if(use_full_data){
    bw_gr = ssvFetchBigwigList(file_paths = bw_files, 
                                    qgr = olaps_fixedSize,
                                    win_size = 50)
}
```
