...
as input.  Sets can be defined in one of several ways.  \code{overlapIntervalSets} creates 
...
\code{overlapIntervalSets} also accepts a GRangesList if that's more natural.
```{r}
olaps_fromGRangesList = overlapIntervalSets(GenomicRanges::GRangesList(CTCF_in_10a_narrowPeak_grs))
```
## Loading Libraries
