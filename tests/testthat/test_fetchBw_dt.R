...
test_that("ssvFetchBigwig return expected for valid even win_size", {
    ...
    for(win in c(2, 6, 10, 30)){
        bw_dt = ssvFetchBigwig(bw_file = test_bw, win_size = win, qgr = test_qgr, return_data.table = TRUE)
...
test_that("ssvFetchBigwig use GRanges names as id", {
    ...
    for(win in c(1, 3, 5, 15)){
        ...
        bw_dt = ssvFetchBigwig(bw_file = test_bw, win_size = win, qgr = qgr, return_data.table = TRUE)
        expect_is(bw_dt, "data.table")
        expect_equal(colnames(bw_dt), exp_colnames)
...
test_that("ssvFetchBigwig throws message if widths aren't divisble by win_size", {
    ...
    for(win in c(7, 31)){
        expect_message(regexp = "widths of qgr were not identical and evenly divisible by win_size", {
            ssvFetchBigwig(bw_file = test_bw, win_size = win, qgr = mix_width_gr, return_data.table = TRUE)
        })
    }
})
...
test_that("ssvFetchBigwig throws message if widths vary", {
    skip_on_os("windows")
    mix_width_gr = test_qgr
