...
# seqsetvis:::ssvFetchBam.single
# seqsetvis:::fetchBam
library(seqsetvis)
library(GenomicRanges)
library(data.table)
pe_file = system.file("extdata/Bcell_PE.mm10.bam", package = "seqsetvis")
# np_f = "/slipstream/home/sethandy/Bcell/peaks/H31_peaks.narrowPeak"
# np_gr = easyLoad_narrowPeak(np_f)[[1]]
# set.seed(0)
# qgr = np_gr[order(np_gr$pValue, decreasing = TRUE)][sample(1:500, 4)]
# qgr = resize(qgr, 600, fix = "center")

data("Bcell_peaks")
qgr = Bcell_peaks

# ssvFetchBam(pe_file, qgr, win_size = 10,
#             target_strand = "both", return_data.table = TRUE, flip_strand = TRUE)

# strand_dt1 = ssvFetchBam(pe_file, qgr, fragLens = NA, win_size = 10,
...
# strand_dt2 = ssvFetchBam(pe_file, qgr, fragLens = 138, win_size = 10,
#                         target_strand = "both", return_data.table = TRUE, flip_strand = FALSE)
...
#
# strand_dt3 = ssvFetchBam(pe_file, qgr, fragLens = NA, win_size = 10,
#                         target_strand = "both", return_data.table = TRUE, flip_strand = TRUE)
...
# strand_dt4 = ssvFetchBam(pe_file, qgr, fragLens = 138, win_size = 10,
#                         target_strand = "both", return_data.table = TRUE, flip_strand = TRUE)
#
...
pe_dt = ssvFetchBamPE(pe_file, qgr, return_data.table = TRUE, win_size = 10)
pe_dt$strand = "pe"
