...
#' @param fill_circle if TRUE, fill circles
...
setPlotBars = function(object) {
  ...
  p <- ggplot(hit_counts_df, aes(x = group, y = count, fill = group)) + labs(x = "") + geom_bar(width = 1, stat = "identity") +
    guides(fill = "none") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), panel.grid.major.x = element_blank(),
...
setPlotPie = function(object) {
  ...
  p <- ggplot(hit_counts_df, aes(x = "", y = count, fill = (group))) + labs(x = "") + geom_bar(width = 1, stat = "identity") +
    guides(x = "none") + theme(axis.text.x = element_blank(), panel.background = element_blank(), axis.ticks = element_blank()) +
    coord_polar("y", start = 0) + scale_y_reverse()
...
setPlotHeatmap = function(df, raster_approximation = T, raster_width_min = 1000, raster_height_min = 1000) {
  ...
  if (raster_approximation) {
    dmat = as.matrix(data.table::dcast(hdt, value.var = "value", formula = rev(row) ~ groups))[, 1 + seq_len(ncol(df)), drop = F]
    low_v = 0.9372549
